...
export function scoreManipulation(x) {
    const reasons = [];
    ...
    if (vwu > 0.15)
        reasons.push({ code: "VOLUME_WITHOUT_USERS", weight: vwu, detail: `1m vol/user=${volPerUser.toFixed(2)} SOL with uniq=${x.uniqBuyers_1m}` });
    ...
    if (uwv > 0.15)
        reasons.push({ code: "USERS_WITHOUT_VOLUME", weight: uwv, detail: `1m uniq=${x.uniqBuyers_1m} with low vol=${vol.toFixed(2)} SOL` });
    ...
    if (x.cadenceMeanSec_1m != null && x.cadenceStdSec_1m != null && x.trades_1m >= 10) {
        ...
        if (botlike > 0.15)
            reasons.push({ code: "CADENCE_BOTLIKE", weight: botlike, detail: `cadence mean=${x.cadenceMeanSec_1m.toFixed(2)}s std=${x.cadenceStdSec_1m.toFixed(2)}s (cv=${cv.toFixed(2)})` });
    }
    // 4) Burst then silence: high 15s, low 5m (or vice versa)
    const burst = x.trades_15s >= 20 && x.trades_5m <= 40;
    if (burst)
        reasons.push({ code: "BURST_THEN_SILENCE", weight: 0.35, detail: `15s trades=${x.trades_15s} but 5m trades=${x.trades_5m}` });
...
export function buildAssessment(mint, symbol, name, features) {
