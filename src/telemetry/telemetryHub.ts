...
import type { TelemetryMessage } from "../types/telemetry.js";
import type {
  CandidateToken,
  CoinAssessment,
  RugAlert,
...
type Subscriber = (msg: TelemetryMessage) => void;

export class TelemetryHub extends BaseService {
  public readonly name = "telemetryHub";

  private subs = new Set<Subscriber>();
  private tickTimer: NodeJS.Timeout | null = null;
  private offAny: (() => void) | null = null;

  constructor(private bus: EventBus, private state: CoreState) { super(); }
  ...
  subscribe(fn: Subscriber) {
    ...
    if (latestAssessment) fn({ type: "assessment", data: latestAssessment });
    const latestSecurity = this.state.securityAlerts[this.state.securityAlerts.length - 1];
    if (latestSecurity) fn({ type: "security_alert", data: latestSecurity });
  ...
  private broadcast(msg: TelemetryMessage) {
    for (const fn of this.subs) {
      try { fn(msg); } catch { /* ignore */ }
    }
  }

  private onEvent(evt: RelayEvent<any>) {
    if (
      evt.type === "PUMPFUN_NEW_TOKEN" ||
  ...
  private latestAssessment() {
    let out: CoinAssessment | undefined;
    for (const a of this.state.coinAssessments.values()) {
      if (!out || a.ts > out.ts) out = a;
    }
    return out;
  }

  private topCandidates(n: number) {
    return Array.from(this.state.candidates.values())
      .sort((a, b) => b.score - a.score)
      .slice(0, n);
  }

  private computeVitalsStub(prev: VitalPoint): VitalPoint {
    const now = Date.now();
  ...
  private estimateTradeCount(prevTrades: number) {
    let max = prevTrades;
    for (const a of this.state.coinAssessments.values()) {
      if (a.features.trades_5m > max) max = a.features.trades_5m;
    }
    return max;
  }
}
