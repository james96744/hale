...
import type { SecurityAlert } from "./types/domain.js";
...
const raydium = new RaydiumWatcher(
  conn,
  bus,
  {
...
async function emergencyPause(alert: SecurityAlert) {
  if (emergencyPauseInFlight) return;
...
supervisor = new Supervisor(
  [telemetry, pumpfun, raydium, txEnricher, coinAssessment, security],
  (line) => log.log(line)
);
...
app.post("/api/start", auth.requireAuth, async (_req, res) => {
  if (state.status === "RUNNING") return res.json({ ok: true, status: state.status });
  ...
  if (!state.analytics.bankrollSOL) state.analytics.bankrollSOL = 10;
  if (!state.analytics.equitySOL) state.analytics.equitySOL = 10;
...
app.post("/api/stop", auth.requireAuth, async (_req, res) => {
  if (state.status === "STOPPED") return res.json({ ok: true, status: state.status });
